...
\begin{definition}[affine spaces, hulls, and independence]
  ...
  The\mpar{
    ...
    \end{tikzpicture}\\
    affine space $\textcolor{setbordercolor}{\mathcal A} = b + \textcolor{altsetbordercolor}{\mathcal V}$ of dimension $1$; $x_1, x_2 ∈ \mathcal A$ are affinely independent%
  }
...
\mpar{
  ...
  \end{tikzpicture}
    a $2$-dimensional polytope (the affine hull is $ℝ^2$) defined by five \textcolor{setbordercolor}{halfspaces}%
  }
...
\mpar{
  ...
  \end{tikzpicture}\\
  the same polytope as the convex hull of its five vertices $x_1,\dotsc,x_5$%
}
...
\mpar{
  ...
  \end{tikzpicture}\\
  two faces induced by valid inequalities with $\dim(\textcolor{Green}{F_1})=0$ and $\dim(\textcolor{Red}{F_2})=1$%
}
...
For a representation  $\P(A,b)$  of the polytope $\P$, each face has another characterization.
...
\mpar{
  ...
  \end{tikzpicture}\\
  example objective $\textcolor{objective}{c}$ such that $c^Tx$ is minimized for $\textcolor{Green}{x^*}$; the dotted lines show the hyperplanes $\textcolor{objective}{c^Tx=0}$ and $\textcolor{objective}{c^Tx = c^T x^*}$%
}
...
\mpar{
  ...
  $\begin{aligned}
    ...
    x_1,\ x_2 &≥ 0
  \end{aligned}$\\[3mm]
  For $\textcolor{DarkCyan}{B=\set 1}$, $A_{•,B}^{-1} b = 2 · 1 = 2$, so $\bar x = (\bar x_B, \bar x_N) = (2,0)$ is the corresponding BFS.
  \begin{tikzpicture}
    \draw [axis] (-.3,0) -- (2.3, 0);
    \draw [axis] (0,-.3) -- (0, 1.3);
...
\mpar{
  \begin{tikzpicture}
    ...
    \draw [->,objective] (0,0) -- (10:6mm) node[above] {$c$};
  \end{tikzpicture}\\
  example run of the primal simplex, starting in $x_2$ and performing \textcolor{path}{three basis exchanges} until the optimal $x_5$ is reached; $\textcolor{objective}{c^T x_i}$ is decreasing on the path%
}
...
\mpar{
  \begin{tikzpicture}
    ...
    \foreach \i [evaluate=\j using {int(mod(\i,5)+1)}] in {1,...,5}
      \draw[setdraw,extend=1.5mm] (x\i) -- (x\j);
  \end{tikzpicture}\\
  feasible set $\textcolor{feasint}{\P ∩ ℤ^2}$ and \textcolor{setbordercolor}{LP relaxation} of an IP%
}
...
The\mpar{
  \begin{tikzpicture}
    ...
    \draw [objline]      (0,0) ++(100:1.1cm) -- ++(100:-1.8cm);
    \draw [objline]      (ip1)  +(100:5mm)   -- +(100:-1cm);
    \node ["$x_\IP^*$" {above left,text=feasint,backwhite}] at (ip1) {};
  \end{tikzpicture}\\
  equivalent LP polytope $\textcolor{feasint}{\PI = \conv(\P ∩ ℤ^2)}$ and optimal IP solution $\textcolor{feasint}{x^*_\IP}$%
}
problem is that, in general, it is hard to derive a description of $\PI$ from $\P$. In fact, IPs are \textsf{NP}-hard to solve in general, whereas we have seen above that linear programming is contained in \textsf{P}.
...
\mpar{
  \begin{tikzpicture}
    ...
    \end{tikzpicture}\\
  example \textcolor{cut}{cutting plane} that cuts the non-integral initial LP solution $x^1$ but is valid for $\textcolor{feasint}{\PI}$; it implies a new LP solution $x^2$ with improved objective value%
}
...
\mpar{
  ...
  \end{tikzpicture}\\
  all integral points in $\P$ are contained in either $\textcolor{Fuchsia}{S_0 = \set{x∈\P\colon x_2 ≥ 1}}$ or $\textcolor{Olive}{S_1 = \set{x∈\P\colon x_2 ≤ 0}}$, the two solutions $\textcolor{Fuchsia}{x^{S_0}}$ and $\textcolor{Olive}{x^{S_1}}$ have larger objective value than $x^S$%
}
...
\mpar{
  ...
  \end{tikzpicture}\\
  an example b\&b enumeration tree; the \textcolor{LawnGreen!70!Black}{leaves} are not further explored because the respective LPs are either infeasible or have integral solutions
...
\mpar{
  \begin{tikzpicture}[margintikz]
    \graph[
      layered layout,layer distance=1.3cm,
      nodes=v,
  ...
  \end{tikzpicture}\\ 
  $Ξ=\{e_1,\dotsc,e_7\}$; feasible are $\{e_1,e_5\}$, $\{e_1,e_4,e_7\}$, $\{e_2,e_6\}$, and $\{e_3, e_7\}$%
}
